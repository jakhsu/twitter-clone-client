...
function App() {
  ...
  const auth = () => {
    ...
    const cryptedName = CryptoJS.AES.encrypt(
      username,
      import.meta.env.VITE_SECRET_KEY
    ).toString();
    const cryptedPswrd = CryptoJS.AES.encrypt(
      password,
      import.meta.env.VITE_SECRET_KEY
    ).toString();
    ...
    <div className="w-full h-screen flex items-center justify-center">
      {next ? (
        <div className="w-[300px] flex flex-col items-center justify-center gap-6">
          ...
          <span className="text-3xl font-bold">Please enter password</span>
          <div
            type="text"
            className="outline-none flex flex-col items-start justify-center rounded h-14 text-lg w-full bg-black opacity-50 border-2 border-gray-600 border-opacity-50 px-4"
          >
          ...
          <button
            onClick={auth}
            className="w-full h-10 flex font-bold items-center justify-center gap-4 bg-white text-black rounded-3xl transition-all hover:bg-[#E6E6E6] "
          >
            {loading ? (
        ...
        <div className="w-[300px] flex flex-col items-center justify-center gap-6">
          <img src={Logo} width="30" alt="" />
          <span className="text-3xl font-bold">Twitter Clone</span>
          ...
          <Link
            to="/register"
            className="w-full h-10 flex items-center justify-center gap-4 bg-[#1d9bf0] text-white rounded-3xl transition-all hover:bg-[#1A8CD8]"
          >
            <span>Register</span>
          </Link>
