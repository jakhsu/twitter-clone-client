...
function App() {
  ...
  return (
    <div id="hashtags" className="w-full bg-[#16181C] rounded-2xl pt-3">
      <span className="font-bold text-xl px-4 pb-2">
      ...
      {tags.map((item, index) => {
        return (
          <div
            key={index}
            className="flex items-start flex-col px-4 py-2 w-full cursor-pointer transition-all hover:bg-[#1D1F23] "
          >
            <div className="flex items-start flex-col w-full">
              <div className="flex justify-between items-center w-full">
                <span className="text-sm text-[#54595D]">Politics</span>
            ...
            <span className="text-sm text-[#54595D] ">
              {" "}
              {Math.floor(Math.random() * (10000 - 1500) + 1500)} Tweet
            </span>
          </div>
        );
      })}
