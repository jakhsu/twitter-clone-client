    ...
    <div id="users" className="w-full bg-[#16181C] rounded-2xl pt-3">
      ...
      {users.map(({ name, username }, index) => {
        return (
          <div
            key={index}
            className="flex items-center justify-start gap-2 px-4 py-3 mt-2 w-full cursor-pointer transition-all hover:bg-[#080808] "
          >
            <img
              src={Me}
              className="!h-[48px] !w-[48px] rounded-full self-start"
              alt=""
            />
